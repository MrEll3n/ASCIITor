...
from src.py.hud import Window, Menu
import src.py.hud as hud
from src.py.camera import Camera
from src.py.items import Weapon, Armor, Food
from src.py.race import Race
from src.py.role import Role
from src.py.enemy import Enemy, create_enemy
import src.py.fight as fight
...
def main(stdscr):
    ...
    def load_game():
        ...
        GAME_X = int(save_arr[4].strip('\n'))
        GAME_Y = int(save_arr[5].strip('\n'))
        CAM_X = int(save_arr[2].strip('\n'))
        CAM_Y = int(save_arr[3].strip('\n'))

        cam = Camera(CAM_WIDTH, CAM_HEIGHT, CAM_X, CAM_Y)

        OCTAVE = int(save_arr[0].strip('\n')) # random.randrange(25, 30)
        SEED = int(save_arr[1].strip('\n'))


        noise = PerlinNoise(octaves=OCTAVE, seed=SEED)  # random.randrange(0, 100000)
        ...
        p = Player(int(save_arr[6].strip('\n')), int(save_arr[7].strip('\n')), save_arr[8].strip('\n'), save_arr[9].strip('\n'), save_arr[10].strip('\n'), int(save_arr[11].strip('\n')), int(save_arr[12].strip('\n')), int(save_arr[13].strip('\n')), int(save_arr[14].strip('\n')), int(save_arr[15].strip('\n')), int(save_arr[16].strip('\n')), int(save_arr[17].strip('\n')), int(save_arr[18].strip('\n')), game_pad, map, int(save_arr[18].strip('\n')))

    
    def new_game(player_name, player_race, player_role, stats_sum):
        global game_menu_bool
        global in_game_bool
        global main_loop
        global char_naming
    ...
    def game():
                    ...
                    case 122: #  z
                        ...
                        match p.inv_lst[inv.highlight - 1][
                            ...
                            case "Armor":
                                ...
                                if not p.inv_lst[inv.highlight - 1][0].strength == 0:
                                    infomenu.print_info(f'STR: {p.inv_lst[inv.highlight - 1][0].strength}')
                                if not p.inv_lst[inv.highlight - 1][0].stamina == 0:
                                    infomenu.print_info(f'STA: {p.inv_lst[inv.highlight - 1][0].stamina}')
                                if not p.inv_lst[inv.highlight - 1][0].dexterity == 0:
                                    infomenu.print_info(f'DEX: {p.inv_lst[inv.highlight - 1][0].dexterity}')
                                if not p.inv_lst[inv.highlight - 1][0].intelligence == 0:
                                    infomenu.print_info(f'INT: {p.inv_lst[inv.highlight - 1][0].intelligence}')
                                if not p.inv_lst[inv.highlight - 1][0].luck == 0:
                                    infomenu.print_info(f'LUC: {p.inv_lst[inv.highlight - 1][0].luck}')
        ...
        def save_game(p):
            if os.path.isfile('src/saves/save_file.txt'):
                os.remove('src/saves/save_file.txt')
            sf = open('src/saves/save_file.txt', 'w')
            ...
            sf.write(f'{p.entity_race}\n')
            sf.write(f'{p.entity_class}\n')
            sf.write(f'{p.entity_lvl}\n')

            sf.write(f'{p.stamina}\n')
            sf.write(f'{p.intelligence}\n')
            sf.write(f'{p.strength}\n')
            sf.write(f'{p.dexterity}\n')
            sf.write(f'{p.defense}\n')
            sf.write(f'{p.luck}\n')
            sf.write(f'{p.carry}\n')

            sf.write(f'{p.inv_lst}\n')
            sf.write(f'{p.wear_lst}\n')
            sf.write(f'{p.hp}\n')




            #sf.write(f'{p.game_pad}\n')
            #sf.write(f'{p.map}\n')
                    ...
                    case 122: #  z
                        match panel_menu.highlight:
                            case 1:
                                save_game(p)
                                game_menu_bool = False
                                infomenu.print_info('[Game Saved]')
        ...
        while in_game_bool:
            ...
            if moved:
                if random.randrange(0, 200) < 1 and (p.floor == 'ˏ' or p.floor == ':'):
                    enemy_stats = create_enemy(p.entity_lvl, True)
                    e = Enemy(enemy_stats['name'], enemy_stats['class'], enemy_stats['lvl'],
                            enemy_stats['dmg'], enemy_stats['sta'], enemy_stats['int'],
                            enemy_stats['str'], enemy_stats['dex'], enemy_stats['def'],
    ...
    def character_creation_confirm():
        ...
        while char_confirm:
            ...
            match key:
                case 122: #  z
                    new_game(player_name, player_race, player_role, stats_sum)
                    game()
    ...
    def character_role_scene(stats_menu):
        ...
        with open('src/json/roles.json', 'r') as f:
    ...
    def character_race_scene():
        ...
        with open('src/json/races.json', 'r') as f:
